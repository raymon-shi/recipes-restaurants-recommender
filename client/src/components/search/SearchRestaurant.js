...
const SearchRestaurant = () => {
  ...
  const { recipeName, starRating, reviewCount } = useParams();

  const navigate = useNavigate();

  const gettingSearchResults = async () => {
    try {
      const { data } = await axios.get('/searchRestaurants', { params: { recipeName, starRating, reviewCount } });
  ...
  return (
    <>
      <Container>
        <h1>{`Restaurant Recommendations based on "${recipeName}"`}</h1>
        <hr />
        {Array(rows)
          .fill()
          .map((_, rowIndex) => (
